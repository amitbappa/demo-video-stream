...
io.on('connection', (socket) => {
  socket.on('join', (roomId) => {
    ...
    if (numberOfClients == 0) {
      ...
      socket.join(roomId)
      socket.emit('room_created', {roomId,YT_VIDEO_ID,YT_API_KEY})
    } else if (numberOfClients == 1) {
      console.log(`Joining room ${roomId} and emitting room_joined socket event`)
      socket.join(roomId)
      socket.emit('room_joined', {roomId,YT_VIDEO_ID,YT_API_KEY})
    } else {
  ...
  socket.on('set_current', function(data) {
        ...
        io.sockets.in(data.roomId).emit('play_video',{roomId:data.roomId,YT_VIDEO_ID:data.YT_VIDEO_ID,isRoomCreator:true})
    })
